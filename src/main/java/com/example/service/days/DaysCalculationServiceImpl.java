...
public class DaysCalculationServiceImpl implements DaysCalculationService {
    ...
    public int calculatePaidDays(int vacationDays,
        ...
        // Выходные дни в отпуске оплачиваются, в случае праздников дата выхода с отпуска просто переносится на количество дней праздника.
        // То есть второго и первого вызова filter не должно быть.
        // Но по заданию должен проводиться рассчет отпускных с учётом праздников и выходных.
        // Если бы была возможность, то этот момент обязательно уточнил!
